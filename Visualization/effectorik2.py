...
class System():
  ...
  def effectors(self):
    ...
    this_effectors = [] 
    this_effectors.append(node.frame().t + node.frame().r*(array([1,0,0])*100 + self.bike_point))
    bike_normal = normalized(self.pointNormal(self.bike_point))
   # print bike_normal
    bike_normal = array([1, 0, 0])
    # bike_normal = normalized(array([-1, -1, 0]))
    this_effectors.append(node.frame().t + node.frame().r*(array([1.,0,0])*100  + self.bike_point + bike_normal*25))
    ...
    # orthogonal_projection = normalized(orthogonal_projection) 
    # this_effectors.append(node.frame().t + node.frame().r*(array([1.,0,0])*100  + self.bike_point + orthogonal_projection*25))  #self.pointNormal(self.bike_point)*100))
    #print magnitude(orthogonal_projection * 100)
   # this_effectors.append(node.frame().t + node.frame().r*(array([0, 0, 1])*100 + self.bike_point))
    # print this_effectors
    # print node.frame().t
   # print self.bike_point
  ...
  def solve_ik(self, initialize):
    ...
    while norm(e) > .1 and iters < 3e2:
      iters+=1
      e = self.targets()-self.effectors()
      # print "These are the effectors: ", self.effectors()
      # print "These ar the targets: ", self.targets()
      # print "And this is how far we are from a solution: ", e, norm(e)
      # print iters
      for i,v in enumerate(e):
        e[i] = self.this_clamp(v, 500.)
      e = ndarray.flatten(e)
...
#This function is used basically just for rendering 
  def bike(self):
