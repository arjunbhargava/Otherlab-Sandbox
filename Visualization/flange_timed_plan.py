...
initial_rotation = pi
location = array([1200, -600, 0])
#location = array([1500, 0, 0])
step1 = 0
step2 = 2

#Initialization of robots
arm2_frame = Frames(location,Rotation.from_angle_axis(initial_rotation,array([0,0,1])))
armset2 = System([Arm('a3',6, effector_functions, bf =arm2_frame)], generate_targets(target_point, array([0, 0, -1]), 0, array([0, 0 , 1]), target_point), 1)
armset_dummy2 = System([Arm('a4',6,effector_functions, bf=arm2_frame)], generate_targets(target_point, array([0, 0, -1]), 0, array([0, 0, 1]), target_point),1)
...
#      sample_axis = axis_r * initial_axis
    ...
    #target_axis = array([1, 1, 0])
    target_axis = array([0, 0, -1])
    print target_axis
    tp = target_point - (300 * target_axis)
    triangle = generate_targets(tp, array([0, 0, 1]), 0, target_axis, tp)
    points.append(triangle[0])
    points.append(triangle[1])
    points.append(triangle[2])
    armset_dummy2.set_target(triangle)
    goal, e = armset_dummy2.solve_ik(counter)
    if magnitude(e) < .01:
...
def solve_2(first_path, normals, targets):
  print "Solving 2"
  global angle_path2
  global location
  ...
  goal_list, angle_goals = setup_2(normals, targets) 
...
def view():
  ...
  angle_path1, normals, targets = getAnglePath(step1, step2)
  #print angle_path1
  #Then given the axes and previous path, solve the second robot. 
  solve_2(angle_path1, normals, targets)
