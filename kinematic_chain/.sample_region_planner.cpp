...
double angle_weights [] = {5, 10, 15, 20, 130, 350};
   		...
   		vector<Vector<real,3>> effectorPositions(const ob::State * state, int effector_flag) const
   		{
   			...
   			if(effector_flag == 1) {
	   			...
	   			for(unsigned int i = 0; i < sys_.robot_number; i++) {
			    		...
			    		end_positions.push_back(effector.t + effector.r * (25 * sys_.axis_information[index].axis));
			    }
...
class  pointNormalGoal : public ob::GoalSampleableRegion
{
	public:

	    pointNormalGoal(const ob::SpaceInformationPtr &si, System &sys, Array<real> goal_angles) : ob::GoalSampleableRegion(si),
	     sys_(sys), space_(new ChainSpace(sys)), goal_angles_(goal_angles)
	    {
	    	setThreshold(sys.tolerance);
	    	rng_ = new ompl::RNG();
	    	checker_ = new SO26ValidityChecker(si, sys);
	    	this_goal = si_->getStateSpace()->allocState();
	    	flag = false;
	    }
	  
	    virtual void sampleGoal(ob::State *st) const
	    {	
			ob::State *copy = si_->getStateSpace()->allocState();
			si_->getStateSpace()->copyState(copy, st);
			int counter = 1; 

			if(!flag) {
				ChainSpace::StateType* cstate = static_cast<ChainSpace::StateType*>(copy); 
				for(unsigned int i = 0; i < sys_.stateDimension; i++) {
		   			cstate->as<ob::SO2StateSpace::StateType>(i)->value = goal_angles_[i];
		   		}
		   		cout << copy;

	 	    	while(!checker_->isValid(copy)) {
		    		cout << "Trying to find a valid goal state" << endl;
					for(unsigned int i = 0; i < sys_.stateDimension; i++) {
						int a = (int) counter/(pow(15, (11-i)%6));
						if(a >0) {
							//double alpha = 1/(6 - i%6);
							double offset =  rng_->uniformReal(-angle_weights[i%6], angle_weights[i%6]) * pi/180;
							double new_angle = goal_angles_[i] + offset;
							if(new_angle > pi) {
							...
							cout << counter << endl;
							cout << i << endl;
		   					cstate->as<ob::SO2StateSpace::StateType>(i)->value = new_angle;
		   					cout << cstate->as<ob::SO2StateSpace::StateType>(i)->value << endl;
		   				}
		   			}
	        	...
	        	si_->getStateSpace()->copyState(st, copy);
	        	si_->getStateSpace()->copyState(this_goal, copy);
	        	si_->getStateSpace()->freeState(copy);
	        	flag = true;
	        	cout << "Got all the way through her" << endl;
	        } else {
	        	si_->getStateSpace()->copyState(st, this_goal);
	        }
	    }
	...
	protected: 
	   	System sys_;
	   	ChainSpace* space_;
	   	Array<real> goal_angles_;
	   	SO26ValidityChecker* checker_;
	   	ob::State* this_goal;
};
...
static Nested<real> plan(unsigned int links, double robot_number, vector<Vector<real,3>> goalState, Array<Vector<real,3>,2> parsed_offsets, 
	...
	double initial_angle, vector<Vector<real, 3>> initial_location, double tolerance, vector<Array<real>> goalAngles) 
{
...
//	ob::ScopedState<ob::CompoundStateSpace> goal(space);
	...
	for(size_t k = 0; k < goalState.size(); k++) {
		...
		//	goal->as<ob::SO2StateSpace::StateType>(index)->value = goalAngles[k][index];
		}

	//	pdef->setStartAndGoalStates(start, goal);
		pdef->addStartState(start);
	   	pdef->setGoal(ob::GoalPtr(new pointNormalGoal(si, sys, goalAngles[k])));
	       ...
	       // 	cout << "Simplified the path? " << endl;
	        ...
	        for(unsigned int i = 0; i < path_states.size(); ++i) {
	        	...
	        	for(unsigned int j = 0; j < si->getStateDimension(); ++j) {
	      		...
	      		cout << asarray(step_vector).copy() << endl;
	      		step_vector.clear();
	        }
	        cout << end_state << endl;
...
vector<Vector<real,3>> sample_function(unsigned int links, double robot_number, vector<Vector<real,3>> goalState, Array<Vector<real,3>,2> parsed_offsets, 
	vector<vector<Ref<TriMesh>>> robotMeshes, vector<Ref<TriMesh>> obstacleMeshes, double resolution, double range, double solve_time, 
	double initial_angle, vector<Vector<real, 3>> initial_location, double tolerance, vector<Array<real>> goal_angles) 
{
	plan( links, robot_number, goalState, parsed_offsets, robotMeshes, obstacleMeshes,  resolution,  range,  solve_time, initial_angle,  initial_location,  tolerance, goal_angles);
	return samples;
}
