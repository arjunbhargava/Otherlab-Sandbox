...
vector<Vector<real,3>> computeJacobian(System &sys, ob::State* state) {
	...
	for(unsigned int i =0; i < sys.stateDimension; i++) {
		...
		if(i < sys.stateDimension/sys.robot_number) 
 		...
 		/*cout << "--------" << endl;
 		cout << delta << endl;
 		cout << world_axis << endl;
		cout << link_positions[i] << endl;*/
...
public:
    ...
    virtual void sampleUniform(ob::State *state)
    {
	    ...
	    double fork = rng_.uniformReal(.2, 1);
	    vector<Vector<real,3>> j;
	    Array<real> joint_angles;

	    if(fork < .2) {
	    	j = computeJacobian(sys_, state);
		    joint_angles = anglesFromState(sys_, state);
		} else {
	    	j = computeJacobian(sys_, closest_state);
		    joint_angles = anglesFromState(sys_, closest_state);	    
	    }

	  	vector<double> changed_distances = getRotatedDistances(frame_from_state(sys_, joint_angles), j);

	   	if(fork < .2) {
	        for(unsigned int i = 0; i < sys_.stateDimension; i++ ) {
    	   		cstate->as<ob::SO2StateSpace::StateType>(i)->value = 
       				cstate->as<ob::SO2StateSpace::StateType>(i)->value + rng_.uniformReal(0, .01 * changed_distances[i]);
       		}
      	 } else { 
        	for(unsigned int i = 0; i < sys_.stateDimension; i++ ) {
        		ChainSpace::StateType* cstate2 = static_cast<ChainSpace::StateType*>(closest_state);
        		cstate->as<ob::SO2StateSpace::StateType>(i)->value = 
        			cstate2->as<ob::SO2StateSpace::StateType>(i)->value + rng_.uniformReal(0,.01 * changed_distances[i]);
			}
		}
    }
...
private:
	vector<double> getRotatedDistances(vector<Frame<Vector<real,3>>> link_frames, vector<Vector<real,3>> j) {
	...
	double initial_angle, vector<Vector<real, 3>> initial_location, double tolerance) 
{
	...
	printVector(Vector<real,3>(0,0,0));
